...
class UserInterface:
    ...
    def __init__(self):
        ...
        for col in range(0,NUM_ROWS):
            for row in range(0, NUM_ROWS):
                if col ==3 and row==3:
                    self.__label_marker= tkinter.Label(self.__main_window,text="",
                                                       height=2,width=5, bg="red").grid(row= row,column=col)
                else:

                    self.__button.append(tkinter.Button(self.__main_window, text=str(DATA.pop()),
    ...
    def switch_value(self, row, column):
        if row - 1 < 0 and column - 1 < 0:
            ...
            self.__button[column][row] = tkinter.Label(self.__main_window,text="",
                                                       height=2,width=5, bg="red").grid(row= row,column=column)
            self.__button[3][3] =
