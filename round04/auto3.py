...
def fill(size_gas, requested_gas, current_gas):
    # if float(size_gas) and float(requested_gas) and float(current_gas):
    if current_gas <= 0:
        if requested_gas >= size_gas:
            return float(size_gas)
        else:
            return float(requested_gas)
    else:
        if current_gas + requested_gas >= size_gas:
            return float(size_gas)
        else:
            return float(current_gas + requested_gas)
...
def drive(x, y, new_x, new_y, current_gas, consumpted_100_gas):
    ...
    p = float((current_gas / consumpted_100_gas) * 100)
    gas_after_driving = float(current_gas - (m / 100) * consumpted_100_gas)
    if gas_after_driving <= 0:
        gas_after_driving = 0.0
    if m <= p:

        return float(gas_after_driving), float(new_x), float(new_y)
    else:
        a, b = coordinate_stop_point(x, y, new_x, new_y, current_gas, consumpted_100_gas)
        return float(gas_after_driving), float(a), float(b)
...
def distance_move(x, y, new_x, new_y):
    return float(sqrt((new_x - x) * (new_x - x) + (new_y - y) * (new_y - y)))
