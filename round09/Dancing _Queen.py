...
def main():
    ...
    for sub_score in sorted(score):
        print(sub_score[0] + ': ')
        for element in sorted(sub_score[1][0::2]):
            temp = '- ' + str(element)
            numerator = 0
            denominator = 0
            total = 0
            for index in range(len(sub_score[1][sub_score[1].index(element) + 1])):
                numerator += sub_score[1][sub_score[1].index(element) + 1][index] * coefficients[
                    index]

            for index in range(len(sub_score[1][sub_score[1].index(element) + 1])):
                denominator += sub_score[1][sub_score[1].index(element) + 1][index] * MAX_EFFICIENT
